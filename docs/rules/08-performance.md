...
## Métricas y baselines (de referencia, a revisar en F1)

- `mat4.mul`: ~34 ns | `mat4.inverse`: ~18 ns | `quat.slerp`: ~75 ns
- `entity.create`: ~8 ns | query 10k: ~170 μs (baselines Zig; C++ debe igualar o mejorar)
- Targets nuevos: `query empty` < 5 μs, `arena alloc` < 60 ns (ROADMAP §7)
...
3. **Data-oriented**: los sistemas iteran arrays planos de componentes; nada de recorrer
   el mundo por punteros dispersos. La jerarquía ECS es de archetypes (ADR-007).
