...
// Struct large enough to exercise the typed allocation path with a 16-byte
// alignment (elementSize must be >= alignment).
struct Vector4 {
    float x{0.0f};
    float y{0.0f};
    float z{0.0f};
...
TEST_CASE("allocate hands out distinct slot addresses") {
    ...
    infinity::core::PoolAllocator pool{16, 16, 3, backing};

    auto* first = pool.allocate(16, 16);
    auto* second = pool.allocate(16, 16);
    auto* third = pool.allocate(16, 16);
...
TEST_CASE("every slot is aligned to the pool alignment") {
    ...
    for (size_t i = 0; i < pool.capacity(); ++i) {
        void* slot = pool.allocate(16, 16);
        CHECK(slot != nullptr);
        CHECK(reinterpret_cast<uintptr_t>(slot) % 16 == 0);
    }
}
...
TEST_CASE("deallocate returns the slot to the free list for immediate reuse") {
    ...
    auto* again = pool.allocate(16, 16);
    CHECK(again == second);
    CHECK(pool.usedCount() == 2);
}
...
TEST_CASE("usedCount and capacity stay correct through mixed sequences") {
    ...
    auto* a = pool.allocate(16, 16);
    auto* b = pool.allocate(16, 16);
    ...
    auto* c = pool.allocate(16, 16);
    auto* d = pool.allocate(16, 16);
    CHECK(c != nullptr);
    CHECK(d != nullptr);
    CHECK(pool.usedCount() == 3);
...
TEST_CASE("two pools built with the same parameters hand out slots in the same order") {
    ...
    infinity::core::PoolAllocator poolA{16, 16, 4, backingA};
    infinity::core::PoolAllocator poolB{16, 16, 4, backingB};

    std::array<void*, 4> slotsA{};
    std::array<void*, 4> slotsB{};
    for (size_t i = 0; i < 4; ++i) {
        slotsA[i] = poolA.allocate(16, 16);
        slotsB[i] = poolB.allocate(16, 16);
    }
...
TEST_CASE("backing is hit once at construction and once at destruction") {
    CountingAllocator backing;
    {
        infinity::core::PoolAllocator pool{16, 16, 8, backing};
        CHECK(backing.allocationCount() == 1);
        CHECK(backing.deallocationCount() == 0);
        (void)pool.allocate(16, 16);
    }
...
TEST_CASE("supportsAlignment accepts only the pool alignment and byte alignment") {
    ...
    CHECK(pool.supportsAlignment(16));
    ...
    CHECK_FALSE(pool.supportsAlignment(8));
    CHECK_FALSE(pool.supportsAlignment(32));
}

TEST_CASE("allocateObject with 16-byte alignment returns an aligned object") {
    CountingAllocator backing;
    infinity::core::PoolAllocator pool{sizeof(Vector4), 16, 4, backing};

    auto* vector = pool.allocateObject<Vector4>(16);

    CHECK(vector != nullptr);
    CHECK(reinterpret_cast<uintptr_t>(vector) % 16 == 0);
    CHECK(pool.usedCount() == 1);
...
TEST_CASE("standalone pool is backed by an internal malloc backend") {
    infinity::core::PoolAllocator pool{16, 16, 4};

    CHECK(pool.capacity() == 4);

    auto* first = pool.allocate(16, 16);
    CHECK(first != nullptr);
    CHECK(reinterpret_cast<uintptr_t>(first) % 16 == 0);
